...
// --- PERBAIKAN: Tipe data baru untuk baris di file Excel ---
...
export default function ManageBattleQuestionsPage() {
  ...
  const handleAddSubject = async (e: FormEvent) => { /* ... (tidak ada perubahan) ... */ };
  const handleDeleteQuestion = async (questionId: string) => { /* ... (tidak ada perubahan) ... */ };
  const handleAddQuestion = async (e: FormEvent) => { /* ... (tidak ada perubahan) ... */ };

  // --- PERBAIKAN: Fungsi untuk menangani unggahan Excel diperbarui ---
  const handleExcelUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        // Buat peta dari nama mapel ke ID untuk pencarian cepat
        ...
        const formattedQuestions = json.map(row => {
          ...
          // Validasi: Pastikan mapel ada
          if (!subjectId) {
            throw new Error(`Mata pelajaran "${row['Mata Pelajaran']}" tidak ditemukan. Harap buat mapel terlebih dahulu.`);
          }
          // Validasi: Pastikan semua kolom wajib ada
        ...
        // Muat ulang daftar soal jika pengguna sedang melihat salah satu mapel yang diunggah
  ...
  return (
    <RoleGuard allowedRoles={['guru']}>
      <div className="max-w-4xl mx-auto p-4 md:p-8">
        ...
        {/* --- UI UNGGAH EXCEL SEKARANG DI ATAS --- */}
        <div className="card mb-8">
            <h2 className="text-xl font-semibold mb-4">Unggah Soal Massal dari Excel</h2>
            <div className="space-y-4">
              <p className="text-sm text-gray-500">
                Cara tercepat untuk menambah banyak soal. Pastikan file Excel Anda memiliki kolom: <strong>Pertanyaan</strong>, <strong>Opsi A</strong>, <strong>Opsi B</strong>, <strong>Opsi C</strong>, <strong>Jawaban Benar (A/B/C)</strong>, <strong>Kelas</strong>, dan <strong>Mata Pelajaran</strong>.
              </p>
              <a href="/template-bank-soal.xlsx" download className="text-sky-500 hover:underline block">
                Unduh File Template Excel
              </a>
              <div>
                <label htmlFor="excel-upload" className="btn btn-success w-full flex items-center justify-center gap-2 cursor-pointer">
                  <UploadCloud size={20} />
              ...
              {uploadStatus && (
                  <div className="mt-4 p-3 bg-slate-100 dark:bg-slate-700 rounded-lg text-sm">
                      <p><strong>Status Unggahan:</strong> {isUploading ? 'Memproses...' : uploadStatus}</p>
                  </div>
              )}
            </div>
        </div>
