...
import { PlusCircle, Trash2, FileText, Image as ImageIcon, Video, AlertCircle } from 'lucide-react';
import RoleGuard from '@/components/RoleGuard'; // <-- 1. Impor RoleGuard Anda
...
export default function CreateCoursePage() {
    ...
    const addContentToStep = (stepIndex: number, contentType: 'text' | 'image' | 'video') => {
        const updatedSteps = [...steps];
        updatedSteps[stepIndex].content.push({
            type: contentType,
            value: '',
            temp_id: `content-${Date.now()}`
        });
        setSteps(updatedSteps);
    };
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (courseDetails.title.trim() === '' || steps.length === 0) {
        ...
        try {
            ...
            setError(`Gagal membuat kursus: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        // <-- 2. Bungkus seluruh JSX dengan RoleGuard -->
        <RoleGuard allowedRoles={['guru']}>
            <div className="p-4 md:p-8 max-w-4xl mx-auto">
                <h1 className="text-3xl font-bold mb-6">Buat Kursus Baru</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    <div className="card p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" value={courseDetails.title} onChange={handleCourseDetailChange} className="input w-full" placeholder="Contoh: Belajar Ekosistem" required />
                        </div>
                        <div>
                            <label htmlFor="description" className="block text-sm font-medium mb-1">Deskripsi Singkat</label>
                            <textarea name="description" id="description" value={courseDetails.description} onChange={handleCourseDetailChange} className="input w-full" rows={3} placeholder="Jelaskan tentang kursus ini"></textarea>
                        </div>
                        <div>
                            ...
                            {steps.map((step, stepIndex) => (
                                <div key={step.temp_id} className="card p-5 border-l-4 border-sky-500">
                                    ...
                                    <div className="space-y-4">
                                        <input type="text" value={step.title} onChange={(e) => handleStepChange(stepIndex, 'title', e.target.value)} className="input w-full" placeholder="Judul Tahapan (Contoh: Apa itu Rantai Makanan?)" />
                                        
                                        <div className="space-y-3 pl-4 border-l-2">
                                            {step.content.map((content, contentIndex) => (
                                                <div key={content.temp_id} className="relative">
                    ...
                    <div className="pt-6 border-t">
                        ...
                        <button type="submit" className="btn btn-primary w-full text-lg" disabled={loading}>
                            {loading ? 'Menyimpan...' : 'Simpan dan Terbitkan Kursus'}
                        </button>
                    </div>
                </form>
            </div>
