...
// --- PERBAIKAN: Komponen Profile sekarang mengambil datanya sendiri ---
export default function Profile() {
  ...
  // State untuk form edit, diinisialisasi sebagai string kosong
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      // Set state profil dan form edit setelah data diterima
      setProfile(profileData as ProfileData);
      setFullName(profileData.full_name || '');
      setKelas(profileData.kelas || '');
      setAvatarUrl(profileData.avatar_url || null);

      // 2. Ambil data statistik berdasarkan peran
  ...
  const handleUpdate = async () => {
    ...
    if (updateError) {
      ...
      // Perbarui state profil dengan data baru dari database
  ...
  // Tampilan loading
  if (loading) {
    return <p className="p-8 text-center">Memuat dasbor Anda...</p>;
  }
  ...
  // Tampilan untuk Edit Profil (dipisahkan agar lebih rapi)
    ...
    <div className="p-4 md:p-6 max-w-lg mx-auto mt-10">
      <div className="card p-6 space-y-6">
        ...
        <div className="flex flex-col items-center space-y-4">
          <img src={avatarUrl || `https://api.dicebear.com/8.x/initials/svg?seed=${fullName}`} alt="Avatar" className="w-32 h-32 rounded-full object-cover border-4 border-gray-200 dark:border-gray-700" />
  ...
  if (profile.role === 'guru') {
    ...
    return (
      <div className="p-4 md:p-8 space-y-8">
        <div><h1 className="text-4xl font-bold">Selamat Datang, Guru {profile.full_name}!</h1><p className="text-gray-500">Berikut adalah ringkasan aktivitas di sekolah Anda.</p></div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <StatCard title="Total Siswa" value={data?.student_distribution?.reduce((sum, item) => sum + item.student_count, 0) || 0} icon={<Users size={28}/>} />
  ...
  return (
    <div className="p-4 md:p-8 space-y-8">
      <div className="flex flex-col md:flex-row items-center gap-6">
        <img src={profile.avatar_url || `https://api.dicebear.com/8.x/initials/svg?seed=${profile.full_name}`} alt="Avatar" className="w-24 h-24 rounded-full border-4 border-sky-500/50" />
        <div>
